visuals = {
	...
	yoloswag: function(mx, my, h, w) {
		svg.append("svg:text")
		.text("#yoloswag")
		.attr("font-size","16").attr("font-family","Lato").attr("fill","#ffffff").attr("fill-opacity",1)
		.attr("x",mx).attr("y",my)
		.transition().duration(500).ease(Math.sqrt).attr("font-size","28").attr("x", mx-50).attr("y",my-50).attr("fill-opacity",0.1)
		.remove();
	},

	hexagon: function(mx, my, h, w) {
		var rotation=0;
		for (var k = 0; k < 6; k++) {
			...
			.transition().duration(500).ease(Math.sqrt).attr("cx",mx - 200).attr("cy", my - 200)
			.transition().duration(500).ease(Math.sqrt).attr("cy",my - 200).attr("cx", mx + 200)
			.transition().duration(500).ease(Math.sqrt).attr("cx",mx + 200).attr("cy", my + 200)
			.transition().duration(500).ease(Math.sqrt).attr("cx",my+200).attr("cy", my - 200)
			.style("stroke-opacity",1e-6).style("fill-opacity", 1e-6).remove();
			rotation+=60;
		}
	},

	triangles: function(mx, my, h, w) {
		var p1init = (mx-20) + " " + (my);
		var p2init = mx + " " + (my-40);
		var p3init = (mx+20) + " " + (my);
		var p1 = (mx-50) + " " + (my-100);
	...
	linestomouse: function(mx, my, h, w) {
		var offsets = [
			[[+1, +30], [0, 0], [+1, +300], [0, 0]],
			[[-1, -30], [0, 0], [-1, -300], [0, 0]],
			[[0, 0], [+1, 0], [0, 0], [+1, +300]],
		...
		for (var i = 0; i < offsets.length; i++) {
			...
			svg.append("svg:line")
				...
				.transition().duration(500).ease(Math.sqrt).attr("x1",mx).attr("y1",my).attr("x2",mx).attr("y2",my).remove();
		}
	},

	circlereverse: function(mx, my, h, w) {
		svg.append("svg:circle")
			.attr("cx",mx).attr("cy",my).attr("r",w/2).style("stroke",colors(++ci))
			.transition().duration(500).attr("r",0).style("stroke-opacity",1e-6).remove();
	},

	basiccircle: function(mx, my, h, w) {
		svg.append("svg:circle")
	...
	miniworks: function(mx, my, h, w) {
		...
		for (var i = 0; i < transforms.length; i++) {
			svg.append("svg:circle")
				...
				.transition()
					.attr("transform","translate("+transforms[i]+")").duration(1000).ease(Math.sqrt).attr("r",25)
					.style("stroke-opacity",1e-6).style("fill-opacity",1e-6).remove();
		}
	},

	fireworks: function(mx, my, h, w) {
